...
//<!-- CSS Spécifiques      (Page Creation Lieu)                               -->
      ...
      } //. >> VOIR si possible de casser cette fonction 
      //        pour avoir le listener sur le body et utiliser 'pace_changed' dans le dispatcher
      //                   et l'initialisation avec tous les élements du DOM
...
//<!-- PAGE-EVALUATION / AVIS > GENERATEUR - FN PRIVATE       (FONCTION) -->
      ...
      function regenerateComment(questionKey, humorAction, dataKey) {
        ...
        if (!selectedRadio) {
          ...
          updateStatus({
            conteneurID:    questionKey, 
            log:        `Impossible de régénérer : aucune note n'est sélectionnée pour ${questionKey}.`, 
        ...
        updateStatus({
          conteneurID:    questionKey, 
          log:        `regenerateComment : questionKey: ${questionKey} / noteRef: ${noteRef}`, 
          questionID: questionKey, 
      ...
      function adjustHumorLevel(action, questionID_forRefonly) {    
        ...
        updateStatus({
          conteneurID:    questionID_forRefonly, 
          log:        `adjustHumorLevel : humourLevel: ${humourLevel}`, 
          questionID: questionID_forRefonly, 
      ...
      function updateAvis(questionID, commentText, dataKey) {                         // Fonction qui met à jour le champ (elle a besoin de questionKey)
        ...
        if (textArea) {
          ...
          updateStatus({
            conteneurID:    questionID,
            log:        `Le champ de commentaire avec l'ID ${textAreaId} n'a pas été trouvé.`,
      ...
      function askForAvis(questionID, noteRef, dataKey) {
        ...
        updateStatus({
          conteneurID:    questionID,
          log:        `askForAvis...[param]questionID:${questionID} , noteref: ${noteRef}`,
          questionID: questionID,
        ...
        google.script.run                                                             // ÉTAPE 1 : Chargement
          .withSuccessHandler( (result) => {
            ...
            google.script.run                                                         // ÉTAPE 2 : Finalisation
              .withSuccessHandler( (result) => {
                ...
                updateStatus({
                  conteneurID:    result.questionID,
                  log:        `askForAvis | 2.receiveAvisAgent[success] : result.questionID:${result.questionID} | result.commentText:${result.commentText} | humourLevel:${humourLevel} | dataKey:${result.dataKey}`,
                  questionID: result.questionID,
                  message:    'Terminé!',
                  type:       'success',
                  isLoading:  false
          ...
          .withFailureHandler(error => {
            updateStatus({
                  conteneurID:    error.questionID,
                  log:        `askForAvis | 1.requestAvisAgent : Fail AI:${error}`,
                  questionID: error.questionID,
      ...
      function initializeSectionListeners(questionID, dataKey) {
        ...
        if (!section) {
          updateStatus({  conteneurID: 'eval', type: 'warn', isLoading: false, log: `initializeSectionListeners | Section non trouvée pour l'ID:  ${section}.`,
            questionID:   questionID, 
            message:      "Veuillez relancer la page.", 
          });
          return;
        }
        ...
        section.addEventListener('change', function(event) {                            // --- 1. Gestion des Événements 'change'  ---
          ...
          if (target.type === 'checkbox' && target.id === 'q5-bonus') {
            ...
            // Logique pour mettre à jour appData.bonus
            appData.bonus = isChecked;

            updateStatus({  conteneurID: 'eval', type: 'info', isLoading: false, log: `appData.bonus: ${appData.bonus}`, 
              questionID:   questionID, 
        ...
        // --- 2. Gestion des Événements 'click' (Boutons d'Humeur) ---
        section.addEventListener('click', function(event) {

          let target = event.target;
          
          // Utilise .closest() pour remonter au bouton >> assure que 'target' est bien le bouton parent
          if (!target.classList.contains('submit-button')) {
            target = target.closest('.submit-button');
          }

          // Vérifie si on a trouvé un bouton d'humeur valide et non désactivé
          if (target && target.closest(`#boutons-humour-${questionID}`) && !target.disabled) {
...
//<!-- PAGE-EVALUATION / PHOTOS > DRAG&DROP - FN PRIVATE      (FONCTION) -->
      ...
      function handleFileSelection() {
        ...
        imageFiles.forEach(file => {
          ...
          if (availableSlots > 0) {
            ...
            updateStatus({  conteneurID: "export", type: 'warn', isLoading: false,
              message:      `Attention: La limite maximale de ${MAX_FILES} photos est atteinte.`, 
              current:      0,
              total:        0
            });
          }
      ...
      function handleCropAndAdd() {
          ...
          // 1. Obtenir le canvas rogné à la taille d'exportation souhaitée (1080x1080)
        ...
        // 2. Convertir le canvas en un objet File (Blob)
        croppedCanvas.toBlob((blob) => {
          if (blob) {
            // Créer un nouvel objet File avec le nom du fichier d'origine et la taille d'export
            const originalName = currentFile.name.replace(/(\.[\w\d_-]+)$/i, '');
            const filename = originalName + `-${EXPORT_SIZE}x${EXPORT_SIZE}.png`;
            
            // Stocker l'URL d'objet du Blob pour la prévisualisation (pour éviter le re-rognage)
            const previewUrl = URL.createObjectURL(blob);

            // Stocker le Blob et le nom pour l'enregistrement Drive
            uploadedFiles.push({
              name: filename,
              blob: blob,
